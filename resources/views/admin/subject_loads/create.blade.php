        ...
        <div class="col-md-8 offset-md-2">
            <div class="card">
                <div class="card-header">
                    <h4 class="mb-0">Assign Subject Load</h4>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.subject-loads.store') }}" method="POST" id="subjectLoadForm">
                        @csrf
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="subject_code" id="subject_code" 
                                           ...
                                           value="{{ old('subject_code') }}" required maxlength="20" placeholder="e.g., CS101">
                                    @error('subject_code')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="subject_name" id="subject_name" 
                                           ...
                                           value="{{ old('subject_name') }}" required placeholder="e.g., Introduction to Computer Science">
                                    @error('subject_name')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="section" id="section" 
                                           ...
                                           value="{{ old('section') }}" required maxlength="10" placeholder="e.g., A, B1, C2">
                                    @error('section')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" name="academic_year" id="academic_year" 
                                           ...
                                           value="{{ old('academic_year', date('Y')) }}" required min="2000" max="2100">
                                    @error('academic_year')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" name="room" id="room" 
                                           ...
                                           value="{{ old('room') }}" maxlength="50" placeholder="e.g., Room 101">
                                    @error('room')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group mb-3">
                                    ...
                                    <textarea name="notes" id="notes" rows="3" 
                                              ...
                                              maxlength="1000" placeholder="Additional notes or comments">{{ old('notes') }}</textarea>
                                    @error('notes')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <div class="form-text">
                        ...
                        <div id="facultyLoadSummary" class="alert alert-info" style="display: none;">
                            <h6><i class="fas fa-user-graduate"></i> Faculty Load Summary</h6>
                            <div id="loadSummaryContent"></div>
                        </div>

                        <div class="form-group d-flex justify-content-between">
                            <a href="{{ route('admin.subject-loads.index') }}" class="btn btn-secondary">
                                <i class="fas fa-arrow-left"></i> Back to Subject Loads
                            </a>
...
function loadFacultySummary() {
    ...
    if (!facultyId || !academicYear || !semester) {
        hideFacultyLoadSummary();
        return;
    }

    fetch('{{ route("admin.subject-loads.faculty-load") }}', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function showFacultyLoadSummary(data) {
    const summaryDiv = document.getElementById('facultyLoadSummary');
    const contentDiv = document.getElementById('loadSummaryContent');
    
    contentDiv.innerHTML = `
        <div class="row">
            <div class="col-md-4">
                <strong>Current Load:</strong><br>
                ${data.total_units} units | ${data.total_hours} hours/week<br>
                ${data.subject_count} subjects
            </div>
            <div class="col-md-8">
                <strong>Current Subjects:</strong><br>
                ${data.loads.map(load => `${load.subject_code} (${load.section}) - ${load.schedule}`).join('<br>')}
            </div>
        </div>
    `;
...
function hideFacultyLoadSummary() {
    document.getElementById('facultyLoadSummary').style.display = 'none';
}
...
['subject_code', 'section', 'academic_year', 'semester', 'schedule_day', 'start_time', 'end_time'].forEach(id => {
    document.getElementById(id).addEventListener('change', checkConflicts);
});

['professor_id', 'academic_year', 'semester'].forEach(id => {
    document.getElementById(id).addEventListener('change', loadFacultySummary);
});
